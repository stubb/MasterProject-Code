...
function getUserMediaFromHere() {
	var constraints = "{ audio: {optional: [   {    sourceId: default  }  ] }, video: {  optional: [   {    minWidth: 1280   },   {   minHeight: 720   },   {    googLeakyBucket: true   },   {    sourceId: default   }  ] }})";
  try {
    doGetUserMedia_(constraints)
  } catch (exception) {
    console.log('getUserMedia says: ' + exception);
  }
}
...
function connectFromHere(server, peer_name) {
  if (peer_name === '') {
    // Generate a random name to distinguish us from other tabs:
    peer_name = 'peer_' + Math.floor(Math.random()*10000);
    console.log('Our name from now on will be ' + peer_name);
  }
  connect(server, peer_name);
}

/* exported negotiateCallFromHere */
function negotiateCallFromHere() {
  // Set the global variables with values from our UI.
  setCreateOfferConstraints();
  setCreateAnswerConstraints();
...
function stopLocalStream() {
  if (typeof global.localStream === 'undefined') {
    console.log('Tried to stop local stream, ' +
...
function addLocalStreamToPeerConnection(peerConnection) {
  if (typeof global.localStream  === 'undefined') {
    console.log('Tried to add local stream to peer connection, but there is no ' +
           'stream yet.');
  }
  try {
    peerConnection.addStream(global.localStream, global.addStreamConstraints);
  } catch (exception) {
    console.log('Failed to add stream with constraints ' +
...
function removeLocalStreamFromPeerConnection(peerConnection) {
  if (typeof global.localStream  === 'undefined') {
    console.log('Tried to remove local stream from peer connection, but there is ' +
           'no stream yet.');
  }
  try {
    peerConnection.removeStream(global.localStream);
  } catch (exception) {
    console.log('Could not remove stream: ' + exception);
  }
  console.log('Removed local stream.');
}
...
function getDevices() {
  ...
  if (typeof(MediaStreamTrack.getSources) === 'undefined') {
    ...
    updateGetUserMediaConstraints();
    console.log('getSources not found, device enumeration not supported');
  }
  ...
  MediaStreamTrack.getSources(function(devices) {
    for (var i = 0; i < devices.length; i++) {
      ...
      if (devices[i].kind === 'audio') {
        ...
        console.log('Device type ' + devices[i].kind + ' not recognized, ' +
...
function sendDataOnChannel(data) {
  if (typeof global.dataChannel  === 'undefined') {
    console.log('Trying to send data, but there is no DataChannel.');
  }
...
function insertDtmf(tones, duration, interToneGap) {
  if (typeof global.dtmfSender === 'undefined') {
    console.log('Trying to send DTMF, but there is no DTMF sender.');
  }
...
function handleMessage(peerConnection, message) {
  ...
  if (parsedMsg.type) {
    ...
    peerConnection.setRemoteDescription(
        sessionDescription,
        function() { console.log('setRemoteDescription'); },
        function(error) { console.log('setRemoteDescription', error); });
    if (sessionDescription.type === 'offer') {
      ...
      peerConnection.createAnswer(
          ...
          function(error) { console.log('createAnswer', error); },
    ...
    peerConnection.addIceCandidate(candidate,
        function() { console.log('addIceCandidate'); },
        function(error) { console.log('addIceCandidate', error); }
    );
    return;
  }
...
function createPeerConnection(stunServer) {
  ...
  try {
    ...
    console.log('Failed to create peer connection: ' + exception + " Constraints: " + JSON.stringify(global.pcConstraints, null, ' '));
  }
...
function setupCall(peerConnection) {
  console.log('createOffer with constraints: ' +
        JSON.stringify(global.createOfferConstraints, null, ' '));
  peerConnection.createOffer(
      setLocalAndSendMessage_,
      function(error) { console.log('createOffer', error); },
...
function createDataChannel(peerConnection, label) {
  ...
  global.dataChannel = peerConnection.createDataChannel(label,
      { reliable: false });
  console.log('DataChannel with label ' + global.dataChannel.label + ' initiated ' +
         'locally.');
...
function closeDataChannel() {
  ...
  console.log('DataChannel with label ' + global.dataChannel.label +
         ' is beeing closed.');
...
function createDtmfSender(peerConnection) {
  if (typeof global.dtmfSender !== 'undefined') {
    console.log('Creating DTMF sender, but we already have one.');
  }
  if (typeof global.localStream === 'undefined') {
    console.log('Creating DTMF sender but local stream is undefined.');
  }
...
function connect(serverUrl, clientName) {
  if (typeof global.ourPeerId !== 'undefined') {
    console.log('connecting, but is already connected.');
  }
  console.log('Connecting to ' + serverUrl + ' as ' + clientName);
  global.serverUrl = serverUrl;
  global.ourClientName = clientName;

  var request = new XMLHttpRequest();
  request.open('GET', serverUrl + '/sign_in?' + clientName, true);
  console.log(serverUrl + '/sign_in?' + clientName);
...
function preparePeerConnection() {
  if (typeof global.peerConnection !== 'undefined') {
    console.log('creating peer connection, but we already have one.');
  }
  global.peerConnection = createPeerConnection(STUN_SERVER);
  console.log('ok-peerconnection-created');
}
...
function addLocalStream() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('adding local stream, but we have no peer connection.');
  }
...
function removeLocalStream() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('attempting to remove local stream, but no call is up');
  }
  removeLocalStreamFromPeerConnection(global.peerConnection);
  console.log('ok-local-stream-removed');
}
...
function toggleRemoteStream(selectAudioOrVideoTrack, typeToToggle) {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Tried to toggle remote stream, but have no peer connection.');
  }
  if (global.peerConnection.getRemoteStreams().length === 0) {
    console.log('Tried to toggle remote stream, but not receiving any stream.');
  }
...
function toggleLocalStream(selectAudioOrVideoTrack, typeToToggle) {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Tried to toggle local stream, but have no peer connection.');
  }
  if (global.peerConnection.getLocalStreams().length === 0) {
    console.log('Tried to toggle local stream, but there is no local stream in ' +
           'the call.');
  }
  var track = selectAudioOrVideoTrack(
...
function hangUp() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('hanging up, but has no peer connection');
  }
  if (getReadyState_() !== 'active') {
    console.log('hanging up, but ready state is not active (no call up).');
  }
  sendToPeer(global.remotePeerId, 'BYE');
  closeCall_();
  global.acceptsIncomingCalls = false;
  console.log('ok-call-hung-up');
}
...
function createDataChannelOnPeerConnection() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Tried to create data channel, but have no peer connection.');
  }
  createDataChannel(global.peerConnection, global.ourClientName);
  console.log('ok-datachannel-created');
}
...
function closeDataChannelOnPeerConnection() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Tried to close data channel, but have no peer connection.');
  }
  closeDataChannel(global.peerConnection);
  console.log('ok-datachannel-close');
}
...
function createDtmfSenderOnPeerConnection() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Tried to create DTMF sender, but have no peer connection.');
  }
  createDtmfSender(global.peerConnection);
  console.log('ok-dtmfsender-created');
}
...
function insertDtmfOnSender(tones, duration, interToneGap) {
  ...
  if (typeof global.dtmfSender === 'undefined') {
    console.log('Tried to insert DTMF tones, but have no DTMF sender.');
  }
  insertDtmf(tones, duration, interToneGap);
  console.log('ok-dtmf-sent');
}
...
function sendToPeer(peer, message) {
  ...
  console.log('Sending message ' + messageToLog + ' to peer ' + peer + '.');
...
function updateVideoElementSize(videoElementId, width, height) {
  ...
  if (width > 0 || height > 0) {
    ...
    if (videoElement.videoWidth > 0 || videoElement.videoHeight > 0) {
      ...
      console.log('Set video element "' + videoElementId + '" size to ' +
             videoElement.width + 'x' + videoElement.height);
    } else {
      console.log('"' + videoElementId + '" video stream size is 0, skipping ' +
             ' resize');
    }
  }
...
function doGetUserMedia_(constraints) {
  ...
  try {
    ...
    console.log('Not valid JavaScript expression: ' + constraints);
  }

  console.log('Requesting doGetUserMedia: constraints: ' + constraints);
...
function negotiateCall_() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Negotiating call, but we have no peer connection.');
  } else if (typeof global.ourPeerId === 'undefined') {
    console.log('Negotiating call, but not connected.');
  } else if (typeof global.remotePeerId === 'undefined') {
    console.log('Negotiating call, but missing remote peer.');
  }
  setupCall(global.peerConnection);
  console.log('ok-negotiating');
}
...
function getUserMediaFailedCallback_(error) {
  console.log('GetUserMedia failed with error: ' + error.name);
}
...
function setLocalAndSendMessage_(sessionDescription) {
  ...
  global.peerConnection.setLocalDescription(sessionDescription,
      function() { console.log('setLocalDescription'); },
      function(error) { console.log('setLocalDescription', error); });
...
function onDataChannelReadyStateChange_() {
  console.log('DataChannel state:' + global.dataChannel.readyState);
...
function getUserMediaOkCallback_(stream) {
  ...
  if (stream.getVideoTracks().length > 0) {
    ...
    stream.getVideoTracks()[0].onended = function() {
      console.log(global.localStream + ' getUserMedia successful but ' +
    ...
    stream.getVideoTracks()[0].onmute = function() {
      console.log(global.localStream + ' MediaStreamTrack.onmute event has fired, ' +
             'no frames to the track.');
    };
    stream.getVideoTracks()[0].onunmute = function() {
      console.log(global.localStream + ' MediaStreamTrack.onunmute event has ' +
               'fired.');
    };
  }
}
...
function checkIfDeviceDropdownsArePopulated_() {
  if (document.addEventListener) {
    ...
    console.log('addEventListener is not supported by your browser, cannot update ' +
...
function registerLocalStorage_(elementId) {
  ...
  if (element.tagName !== 'INPUT') {
    console.log('You can only use registerLocalStorage_ for input elements. ' +
  ...
  if (element.type === 'checkbox') {
    ...
    console.log('Unsupportered input type: ' + '\"' + element.type + '\"');
  }
}
...
function getEvaluatedJavaScript_(stringRepresentation) {
  try {
    ...
    console.log('Not valid JavaScript expression: ' + stringRepresentation);
  }
}
...
function toggle_(track, localOrRemote, audioOrVideo) {
  if (!track) {
    console.log('Tried to toggle ' + localOrRemote + ' ' + audioOrVideo +
                 ' stream, but has no such stream.');
  }
  track.enabled = !track.enabled;
  console.log('ok-' + audioOrVideo + '-toggled-to-' + track.enabled);
}

function connectCallback_(request) {
  console.log('Connect callback: ' + request.status + ', ' + request.readyState);
  if (request.status === 0) {
    console.log('peerconnection_server doesn\'t seem to be up.');
    console.log('failed connecting to peerConnection server');
  }
...
function parseRemotePeerIdIfConnected_(responseText) {
  ...
  for (var i = 0; i < peerList.length; i++) {
    ...
    if (id !== global.ourPeerId  ) {
      console.log('Found remote peer with name ' + name + ', id ' +
                id + ' when connecting.');
      // There should be at most one remote peer in this test.
      if (remotePeerId !== null) {
        console.log('Expected just one remote peer in this test: ' +
               'found several.');
      }
      // Found a remote peer.
      remotePeerId = id;
...
function hangingGetCallback_(hangingGetRequest, server, ourId) {
  ...
  if (hangingGetRequest.status !== 200) {
    console.log('Error ' + hangingGetRequest.status + ' from server: ' +
...
function hangingGetTimeoutCallback_(hangingGetRequest, server, ourId) {
  console.log('Hanging GET times out, re-issuing...');
  hangingGetRequest.abort();
  restartHangingGet_(server, ourId);
}
...
function handleServerNotification_(message) {
  ...
  if (parseInt(parsed[2]) === 1) {
    ...
    console.log('Found remote peer with name ' + parsed[0] + ', id ' + parsed[1] +
           ' when connecting.');
...
function closeCall_() {
  if (typeof global.peerConnection === 'undefined') {
    console.log('Closing call, but no call active.');
  }
...
function handlePeerMessage_(peerId, message) {
  ...
  if (peerId !== global.remotePeerId) {
    console.log('Received notification from unknown peer ' + peerId +
           ' (only know about ' + global.remotePeerId + '.');
  }
  if (message.search('BYE') === 0) {
    console.log('Received BYE from peer: closing call');
    closeCall_();
    return;
  }
  if (typeof global.peerConnection  === 'undefined' &&
    ...
    console.log('We are being called: answer...');

    global.peerConnection = createPeerConnection(STUN_SERVER);

    if ($('auto-add-stream-oncall') &&
        obtainGetUserMediaResult_() === 'ok-got-stream') {
      console.log('We have a local stream, so hook it up automatically.');
      addLocalStreamToPeerConnection(global.peerConnection);
    }
...
function readResponseHeader_(request, key) {
  ...
  if (value === null || value.length === 0) {
    console.log('Received empty value ' + value +
